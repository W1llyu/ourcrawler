...
func (p *Processor) Process(response *downloader.Response) (collector.ResultItems, error) {
	...
	doc.Find("a.play-list-link").Each(func(i int, s *goquery.Selection) {
		liveRoom := processEach(s)
		if liveRoom.AudienceCount != 0 {
			liveRooms = append(liveRooms, liveRoom)
		}
	})
	livesource.PackResultItem(&resultItem, liveRooms, strings.TrimSpace(doc.Find("div.listcustomize-topcon-msg").Find("h1").Text()), "斗鱼TV")
	return resultItem, nil
}

func processEach(s *goquery.Selection) livesource.LiveRoom {
	liveRoom := livesource.LiveRoom{}
	liveRoom.Title = strings.TrimSpace(s.Find("div.mes-tit").Find("h3.ellipsis").Text())
	liveRoom.Anchor = strings.TrimSpace(s.Find("span.dy-name").Text())
	liveRoom.AudienceCount = util.ZhNumToEn(strings.TrimSpace(s.Find("span.dy-num").Text()))
	return liveRoom
}
